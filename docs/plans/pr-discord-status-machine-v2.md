...
`fix(discord): harden reaction status state machine (v2 follow-up to #18248)`
...
- Problem:
  - v1 (authored by **@victorGPT**, merged as PR #18248) solved the baseline state transition behavior, but production stress scenarios still exposed semantic jitter/rollback patterns (notably `👀 ↔ 🧠` oscillation and `⏳ -> 💻 -> ⏳` fallback), plus deferred lifecycle edge cases.
  - v1 also did not provide robust, explicit per-message waiting signals for bursty multi-message queues in all edge timings.
- Why it matters:
  - These regressions make in-chat status unreliable under queued/deferred load and can mislead operators about whether a message is waiting vs actively executing.
- What changed:
  - Introduced v2 hardening with global semantic-phase constraints, unified deferred queue outcomes, explicit cleanup for clear/cancel-before-start paths, stricter off-mode behavior handling under deferred lifecycle, and clearer queued-message waiting visibility in burst scenarios.
...
- v1 PR (authored by @victorGPT): https://github.com/openclaw/openclaw/pull/18248

## User-visible / Behavior Changes

- v2 explicitly prevents semantic rollback after ACTIVE begins (e.g., blocks `ACTIVE -> WAITING` regression).
- ACTIVE internal switching (thinking/tool variants) remains valid and is not treated as rollback.
- In multi-message bursts, queued messages now have explicit waiting semantics until they actually start running (better queue-order visibility for users/reviewers).
...
| Rollback handling | Could still show jitter (`👀 ↔ 🧠`, `⏳ -> 💻 -> ⏳`) in edge timing | Global semantic constraints prevent rollback paths |
| Deferred queue outcomes | Partial handling | Unified outcome model (`queued/skipped/dropped/merged/failed`) with cleanup hooks |
| Multi-message queue visibility | Waiting indication could be ambiguous under burst timing | Explicit per-message waiting semantics for queued items until real execution starts |
